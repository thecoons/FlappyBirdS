...
//Charge les images fournis dans source
...
var Pipe = function(json){
	...
	this.width = 50;
	this.height = 40;
	this.speed = game_param.pipeSpeed;
	this.cpt = 0;
	this.direction = 1;
	this.init(json);
}
...
Pipe.prototype.update = function(){
	this.cpt +=1;
	this.x -= this.speed;
	this.y += Math.sin(this.cpt/game_param.freq)*game_param.coef*this.direction;
}
...
Game.prototype.update = function(){
	...
	for(var i in this.birds){
		if(this.birds[i].alive){
			...
			if(this.birds[i].isDead(this.height, this.pipes)){
				...
				if(this.isItEnd()){
					logs.push({x:this.generation,y:this.score})
					this.start();
				}
			}
		}
	...
	if(this.interval == 0){
		var deltaBord = 50;
		var pipeHoll = parseInt(game_param.pipeHoll);
		var direction = Math.random() >= 0.5? -1:1;
		var hollPosition = Math.round(Math.random() * (this.height - deltaBord * 2 - pipeHoll)) +  deltaBord;
		this.pipes.push(new Pipe({x:this.width, y:0, height:hollPosition, direction:direction}));
		this.pipes.push(new Pipe({x:this.width, y:hollPosition+pipeHoll, height:this.height, direction:direction}));
	}
